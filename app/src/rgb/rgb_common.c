...
// TODO: remove
#define CONFIG_ZMK_RGB_UNDERGLOW_HUE_STEP 5
#define CONFIG_ZMK_RGB_UNDERGLOW_SAT_STEP 10
#define CONFIG_ZMK_RGB_UNDERGLOW_BRT_STEP 2
...
#define CONFIG_ZMK_RGB_UNDERGLOW_ON_START true
...
static struct led_rgb layer_pixels[STRIP_NUM_PIXELS];
...
static void zmk_rgb_underglow_effect_layer_matrix() {
    for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
        pixels[i] = layer_pixels[i];//hsb_to_rgb(hsb_scale_min_max(state.color));
    }
}
...
static void zmk_rgb_underglow_tick(struct k_work *work) {
    ...
    case UNDERGLOW_EFFECT_SOLID:
        ...
        zmk_rgb_underglow_effect_layer_matrix();
        break;
...
// #endif
    ...
    {
        // color.h = 20; color.s = 100; color.b =  2;
        layer_pixels[pixel] = hsb_to_rgb(state.color);
    }
...
//     return k_delayed_work_submit(&underglow_save_work, K_MSEC(1000));
// #else
    return 0;
// #endif
}
...
int zmk_rgb_underglow_set_hsb_matrix(struct zmk_led_hsb* color) {
    // if (color.h > HUE_MAX || color.s > SAT_MAX || color.b > BRT_MAX) {
    //     return -ENOTSUP;
    // }
    ...
    for (int pixel = 0; pixel < STRIP_NUM_PIXELS; pixel++)
    {
        layer_pixels[pixel] = hsb_to_rgb(color[pixel]);
    }
...
struct zmk_led_hsb zmk_rgb_underglow_get_color() {
    struct zmk_led_hsb color = state.color;
    return color;
}
